...
from application.models import Post, FavoritePokemon  # Import model FavoritePokemon
...
def profile():
    ...
    user_favorite_pokemons = [fav.pokemon_id for fav in FavoritePokemon.query.filter_by(user_id=current_user.id)]
    # Print post information and favorite Pokemon to the console
    for post in user_posts:
        print(f"Post ID: {post.id}, Author ID: {post.author_id}, Photo: {post.photo}")
    print(f"Favorite Pokemons: {user_favorite_pokemons}")
    page = request.args.get('page', 1, type=int)
    all_posts = Post.query.order_by(Post.post_date.desc()).paginate(page=page, per_page=3)

    return render_template('profile.html', title=f'{current_user.fullname} Profile', posts=user_posts, all_posts=all_posts, favorite_pokemons=user_favorite_pokemons)
...
@app.route('/add_favorite_pokemon', methods=['POST'])
@login_required
def add_favorite_pokemon():
    ...
    new_favorite = FavoritePokemon(user_id=user.id, pokemon_id=pokemon_id)
    db.session.add(new_favorite)
    db.session.commit()

    return jsonify({'success': True, 'message': 'Pokémon added to favorites'}), 200
